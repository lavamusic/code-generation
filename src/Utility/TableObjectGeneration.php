...
class TableObjectGeneration
{
    ...
    public function generationTable()
    {
        ...
        foreach ($this->indexList as $key => $columnName) {
            $table->normal($columnName, $columnName);
        }
        return $table;
    }
    ...
    {
        $query = new QueryBuilder();
        $query->raw("show index from {$this->tableName}");
        ...
        if ($this->connection->getConfig()->isFetchMode()) {
            ...
            $indexData = $data->getResult();
        }
        foreach ($indexData as $indexDatum) {
            $this->indexList[$indexDatum['Column_name']] = $indexDatum['Column_name'];
        }
